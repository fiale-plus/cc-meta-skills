...
<!-- Invocation metadata:
  paths: <comma-separated directory patterns>
...
paths = unique(paths)
paths = generalize_patterns(paths)  // src/api/routes/, src/api/controllers/ → src/api/
```
...
keywords.add_all(extract_terms(description))  // "endpoint", "route", "HTTP"
